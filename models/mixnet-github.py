...
class GroupedConv2D(nn.Module):
    def __init__(self, channels, kernel_size, stride, dilation=1):
        ...
        self._convs = []
        self._splits = _SplitChannels(channels, self._groups)
        for i in range(self._groups):
            self._convs.append(Conv2dSamePadding(
                    in_channels=self._splits[i], 
                    out_channels=self._splits[i], 
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride, expand_ratio, non_linear='ReLU', se_ratio=0.0):
        ...
        if expand:
            ...
            pw_expansion = nn.Sequential(
                ...
                nn.BatchNorm2d(expand_channels,momentum=self._bn_momentum, eps=self._bn_eps),
                NON_LINEARITY[non_linear]
            )
